...
public class Memory implements MemoryInterface {
     ...
     * 段页：
     * 如果数据段已经在内存，使用全关联映射+LRU加载物理页框；
     * 如果数据段不在内存，先按照分段模式进行管理，分配的段长度为数据段包含的总物理页框数/2，再将物理页框加载到内存
     */
     ...
     * 主存大小 32 MB
     */
    public static int MEM_SIZE_B = 32 * 1024 * 1024;

    /**
     * 页大小 1 KB
     */
    public static int PAGE_SIZE_B = 1 * 1024;
    ...
    /**
     * 页表大小为2^16  64K 128*1024个页表项
     */
    private static PageItem[] pageTbl = new PageItem[Disk.DISK_SIZE_B / Memory.PAGE_SIZE_B];

    /**
     * 反向页表大小为2^15   32K
     */
    private static ReversedPageItem[] reversedPageTbl = new ReversedPageItem[Memory.MEM_SIZE_B / Memory.PAGE_SIZE_B];
    ...
    private final HashMap<String, String> stack = new HashMap<>();
    ...
    public String seg_load(int segIndex) {
        ...
        // code here
        int len = this.chars2int(targetSeg.getLimit());
        while (MEM_SIZE_B - this.occupiedSize() < len) {
            this.removeSegByLRU();
        }
        ...
        if (Memory.PAGE) {
            ...
            this.fit(segIndex, null, len);
        } else {
            // Code here
            char[] data = disk.read(String.valueOf(targetSeg.getDisk()), len);
            this.fit(segIndex, data, len);
        }
    ...
    public String page_load(int segIndex, int pageNO) {
        ...
        int NUM = 512;
        // 段的起始内存地址
        int startMemoryAddressNumber = Integer.parseInt(String.valueOf(segTbl.get(segIndex).getBase()), 2);
        // 段的长度
        int segmentLength = Integer.parseInt(String.valueOf(segTbl.get(segIndex).getLimit()), 2);
        // 段中第一个物理页的编号
        int startPhysicalPageNumber = startMemoryAddressNumber / PAGE_SIZE_B;
        // 段中最后一个物理页的编号
        int endPhysicalPageNumber = (startMemoryAddressNumber + segmentLength) / PAGE_SIZE_B - 1;
        int aimPhysicalPageNumber = -1;
        int len = NUM < segmentLength ? NUM : segmentLength;
        char[] data = disk.read(t.intToBinary(String.valueOf(pageNO * PAGE_SIZE_B)), PAGE_SIZE_B * len);
        String res = "";
        */
        /*
        for (int time = 0; time < len; time++) {
            for (int i = startPhysicalPageNumber; i <= endPhysicalPageNumber; i++) {
                if (!reversedPageTbl(i).isValid) {
                    aimPhysicalPageNumber = i;
                    break;
                }
                if (i == endPhysicalPageNumber) {
                    aimPhysicalPageNumber = this.removePageByLRU(startPhysicalPageNumber, endPhysicalPageNumber);
                }
            }
            String physicalPageAddress = t.intToBinary(String.valueOf(aimPhysicalPageNumber * PAGE_SIZE_B));
            if (time == 0) {
                res = physicalPageAddress;
            }
            pageTbl(pageNO + time).setInMem(true);
            pageTbl(pageNO + time).setFrameAddr(physicalPageAddress.toCharArray());
            reversedPageTbl(aimPhysicalPageNumber).isValid = true;
            reversedPageTbl(aimPhysicalPageNumber).vPageNO = pageNO + time;
            reversedPageTbl(aimPhysicalPageNumber).updateTimeStamp();
            tlb.writeTLB(int2chars(pageNO + time), physicalPageAddress.toCharArray());
            this.write(physicalPageAddress, PAGE_SIZE_B, Arrays.copyOfRange(data, time * PAGE_SIZE_B, (time + 1) * PAGE_SIZE_B));
        }
        return res;
         */
        /*
        String physicalPageAddress = "";
        for (int time = 0; time < len; time++) {
            if (time == 0) {
                for (int i = startPhysicalPageNumber; i <= endPhysicalPageNumber; i++) {
                    if (!reversedPageTbl(i).isValid) {
                ...
                res = t.intToBinary(String.valueOf(aimPhysicalPageNumber * PAGE_SIZE_B));
            }
            physicalPageAddress = t.intToBinary(String.valueOf(aimPhysicalPageNumber * PAGE_SIZE_B));
            int row;
            if ((row = tlb.isMatch(pageNO + time)) != -1) {
                tlb.invalid(row);
            }
            pageTbl(pageNO + time).setInMem(true);
            pageTbl(pageNO + time).setFrameAddr(physicalPageAddress.toCharArray());
            ...
            reversedPageTbl(aimPhysicalPageNumber).updateTimeStamp();
            tlb.writeTLB(int2chars(pageNO + time), physicalPageAddress.toCharArray());
            aimPhysicalPageNumber++;
            if (time < len - 1) {
                aimPhysicalPageNumber++;
                if (aimPhysicalPageNumber > endPhysicalPageNumber) {
                    // aimPhysicalPageNumber = this.removePageByLRU(startMemoryAddressNumber, endPhysicalPageNumber);
                    aimPhysicalPageNumber = startPhysicalPageNumber;
                }
            }
        }
        this.write(res,PAGE_SIZE_B * len, data);
        return res;
         */
        /*
        for (int i = startPhysicalPageNumber; i <= endPhysicalPageNumber; i++) {
        ...
        String physicalPageAddress = t.intToBinary(String.valueOf(aimPhysicalPageNumber * PAGE_SIZE_B));
        pageTbl(pageNO).setInMem(true);
        pageTbl(pageNO).setFrameAddr(physicalPageAddress.toCharArray());
        reversedPageTbl(aimPhysicalPageNumber).isValid = true;
        reversedPageTbl(aimPhysicalPageNumber).vPageNO = pageNO;
        reversedPageTbl(aimPhysicalPageNumber).updateTimeStamp();
        tlb.writeTLB(int2chars(pageNO), physicalPageAddress.toCharArray());
        this.write(physicalPageAddress, PAGE_SIZE_B, data);
        return physicalPageAddress;
        */

        int pageNumber = Integer.valueOf(t.binaryToInt("0" + String.valueOf(getLimitOfSegDes(segIndex)))) / PAGE_SIZE_B;
        int len = 512;
        len = pageNumber > len ? len : pageNumber;
        char[] data = disk.read(t.intToBinary(String.valueOf(pageNO * PAGE_SIZE_B)), PAGE_SIZE_B * len);
        int startMemoryAddressNumber = Integer.parseInt(String.valueOf(getBaseOfSegDes(segIndex)), 2);
        for (int i = len - 1; i >= 0; i--) {
            if (tlb.isMatch(reversedPageTbl(startMemoryAddressNumber / PAGE_SIZE_B + i).vPageNO) != -1) {
                tlb.invalid(tlb.isMatch(reversedPageTbl[startMemoryAddressNumber / PAGE_SIZE_B + i].vPageNO));
            }
            pageTbl(pageNO + i).frameAddr = int2chars(startMemoryAddressNumber + i * PAGE_SIZE_B);
            reversedPageTbl(startMemoryAddressNumber / PAGE_SIZE_B + i).vPageNO = pageNO + i;
            reversedPageTbl(startMemoryAddressNumber / PAGE_SIZE_B + i).isValid = true;
            reversedPageTbl(startMemoryAddressNumber / PAGE_SIZE_B + i).timeStamp = System.currentTimeMillis();
            tlb.writeTLB(int2chars(pageNO + i), int2chars(startMemoryAddressNumber + i * PAGE_SIZE_B));
            this.write(String.valueOf(int2chars(startMemoryAddressNumber + i * PAGE_SIZE_B)), PAGE_SIZE_B, Arrays.copyOfRange(data, i * PAGE_SIZE_B, (i + 1) * PAGE_SIZE_B));
        }
        // this.write((String.valueOf(int2chars(startMemoryAddressNumber))), PAGE_SIZE_B * len, data);
        return String.valueOf(int2chars(startMemoryAddressNumber));
